...
if (isset($pref['path_itemstats']))
    define("path_itemstats", $pref['path_itemstats']);
else
{
    if (file_exists(e_BASE . '/e107_config_itemstat.php'))
        include_once(e_BASE . '/e107_config_itemstat.php');
    else
...
function itemstats_parse($message)
{
	...
	while (preg_match('#\[(item)(=[0-5])?\](.+?)\[/item\]#s', $message, $match) OR preg_match('#\[(itemico)(=[0-5])?\](.+?)\[/itemico\]#s', $message, $match))
	{
        ...
        if (defined(path_itemstats))
            $item_html = str_replace("{PATH_ITEMSTATS}", e_BASE . "/" . path_itemstats, $item_html);
        else
            $item_html = str_replace("{PATH_ITEMSTATS}", e_BASE . "/itemstats", $item_html);
        //=====================================================================
