...
class ParseAllakhazam
{
	...
	function getItem($name)
    {
		...
		for ($ct = 0; isset($GLOBALS["allakhazam_lang"][$ct]); $ct++)
		{
			...
			// Perform the search.
			$data = itemstats_read_url('http://wow.allakhazam.com/search.html?q=' . urlencode(utf8_encode($name)) . "&locale=" . $current_lang);
			...
			if (preg_match_all('#images/icons/(.*?)\.png(.*?)witem=(.*?)&amp;.*?">#s', $data, $matches))
			{
				foreach ($matches[0] as $key => $match)
				{
					...
					$xml_data = itemstats_read_url('http://wow.allakhazam.com/dev/wow/item-xml.pl?witem=' . $item_id . "&locale=" . $current_lang);
					
					// Parse out the name of this item from the XML and check if we have a match
					$xml_name = utf8_decode($this->xml_helper->parse($xml_data, 'name1'));
				    ...
				    if (strcasecmp($item['name'], $xml_name) == 0)
					{
						...
						$item['link'] = 'http://wow.allakhazam.com/db/item.html?witem=' . $item_id . "&locale=" . $current_lang;
					    $item_found = true;
						break;
					}
				}
	        ...
	        if ($item_found)
			{
		        $html_data = utf8_decode(itemstats_read_url('http://wow.allakhazam.com/ihtml?' . $item_id . "&locale=" . $current_lang));
				...
				if (!empty($item_set_id) && ($item_set_id != '0'))
				{
					...
					$data = utf8_decode(itemstats_read_url('http://wow.allakhazam.com/db/itemset.html?setid=' . $item_set_id . "&locale=" . $current_lang));
    ...
    {
		// Retrieve the XML for this item from Allakhazam.
		$xml_data = itemstats_read_url('http://wow.allakhazam.com/dev/wow/item-xml.pl?witem=' . $item_id . "&locale=" . $locale_lang);
        ...
        $item['lang'] = 'en';
		$item['icon'] = $matches[1];
		$item['link'] = 'http://wow.allakhazam.com/db/item.html?witem=' . $item_id . "&locale=" . $locale_lang;

		// Parse out the display html of this item from the XML
		$html_data = utf8_decode(itemstats_read_url('http://wow.allakhazam.com/ihtml?' . $item_id . "&locale=" . $locale_lang));
		$intPositionBody = strpos($html_data, '<body>', 0);
        $intPositionEndHTML = strpos($html_data, '<div class="credit">', 0);
        if ($intPositionEndHTML === false)
        {
		...
		{
			// Read the item set page.
			$data = utf8_decode(itemstats_read_url('http://wow.allakhazam.com/db/itemset.html?setid=' . $item_set_id . "&locale=" . $locale_lang));
